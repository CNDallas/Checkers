...
class Chat extends Component {
  ...
  sendMessage = value => {
    ...
    socket.emit(SEND_MESSAGE, message, userMessage => {
      ...
      if (
        this.state.chatMessages.filter(e => e.id === userMessage.id).length ===
        0
      ) {
  ...
  messageReceiver = () => {
    ...
    socket.on(RECEIVE_MESSAGE, recMessage => {
      if (
        this.state.chatMessages.filter(e => e.id === recMessage.id).length === 0
      ) {
  ...
  messageOnClickHandler = (userName, message) => {
    const output = (
      <FullMessage
        show={true}
        handleClose={this.closeMessageBoxHandler}
        message={message}
        username={userName}
      />
    );
    ...
    let chat = chatMessages.map(m => (
      <li key={m.id} style={{ color: m.color }} className="chat-line">
        <span
          ...
          onClick={this.userNameOnClickHandler.bind(this, m.userName)}
        >
          {m.userName}
        </span>
        <span
          className="chat-message"
          onClick={this.messageOnClickHandler.bind(this, m.userName, m.message)}
        >
          {m.message}
        </span>
      </li>
        ...
        {isChatOpen ? (
          <div className="chat-open">
            <div className="chat-header">
              <span className="btn-close" onClick={this.toggleChatHandler}>
                X
              </span>
              Chat
            </div>
          ...
          <button className="toggle-chat" onClick={this.toggleChatHandler}>
            Chat
          </button>
        )}
      </React.Fragment>
